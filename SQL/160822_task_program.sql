...
CREATE TABLE `program` (
  ...
  CONSTRAINT `FK_PROGRAM$CLIENT_ID` FOREIGN KEY (`CLIENT_ID`) REFERENCES `person` (`idPerson`),
  CONSTRAINT `FK_PROGRAM$OWNER_ID` FOREIGN KEY (`OWNER_ID`) REFERENCES `person` (`idPerson`),
  CONSTRAINT `FK_PROGRAM$PRODUCER_ID` FOREIGN KEY (`PRODUCER_ID`) REFERENCES `person` (`idPerson`),
  CONSTRAINT `FK_PROGRAM$ROOM_ID` FOREIGN KEY (`ROOM_ID`) REFERENCES `room` (`idRoom`)
...
CREATE TABLE `task` (
  ...
  CONSTRAINT `FK_TASK$OWNER_ID` FOREIGN KEY (`OWNER_ID`) REFERENCES `person` (`idPerson`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
